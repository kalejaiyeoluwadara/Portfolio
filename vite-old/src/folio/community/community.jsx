    ...
    <div className="relative w-full mb-8 sm:mb-12">
      ...
      <div className="timeline-node">
        <motion.div
          ...
          className={`timeline-node-dot ${colors.bg} ${colors.text}`}
        />
      </div>
      ...
      <div
        className={`
          ...
          ${isLeft ? "md:pr-[calc(50%+32px)] md:text-right" : "md:pl-[calc(50%+32px)] md:text-left"}
          w-full
        `}
      >
        <motion.div
          ...
          className={`
            timeline-card relative rounded-2xl p-5 sm:p-6 border cursor-pointer
            ${darkmode
              ? `bg-slate-900/60 border-slate-700/50 timeline-card-dark`
              : `bg-white/70 border-slate-200 timeline-card-light`
            }
          `}
          onClick={onToggle}
        >
          {/* Number badge */}
          <span
            className={`absolute top-4 ${isLeft ? "right-4 md:left-4 md:right-auto" : "right-4"} 
          ...
          {/* Title */}
          <h3
            className={`font-bold text-base sm:text-lg leading-snug pr-10 sm:pr-12
  ...
  return (
    ...
    >
      <div className="w-[90%] sm:w-[80%] relative flex flex-col items-center gap-10">
        <Head h1={"Community"} p={"Impact & educational initiatives"} />
        ...
        >
          ...
          <button
            ...
            className={`relative px-5 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 select-none outline-none ${
              activeCategory === "all"
                ? "text-white"
                : darkmode
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`relative px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 select-none outline-none flex items-center gap-1.5 ${
                  isActive
                    ? "text-white"
                    : darkmode
                    ? "text-slate-400 hover:text-slate-200"
        ...
        <div className="relative w-full mt-4">
          ...
          <div
            className={`timeline-line ${
              darkmode ? "timeline-line-dark" : "timeline-line-light"
            }`}
          />
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            {impactStats.map((stat, i) => (
              ...
              >
                ...
                <span
                  className={`text-xs sm:text-sm font-semibold mt-1 opacity-70 text-center ${
          ...
          <div className={`space-y-2 text-sm leading-relaxed ${
            ...
            <p>
              <span className={`font-semibold ${darkmode ? "text-slate-200" : "text-slate-800"}`}>11 digital products</span> serving students, event organizers, and university communities.
            </p>
            <p>
              Solutions spanning <span className={`font-semibold ${darkmode ? "text-slate-200" : "text-slate-800"}`}>event management, elections, certification, helpdesk systems, community engagement, and educational technology.</span>
            </p>
            <p>
              Led projects from <span className={`font-semibold ${darkmode ? "text-slate-200" : "text-slate-800"}`}>concept and UI/UX design to development, deployment, and maintenance.</span>
            </p>
          </div>
        </motion.div>
      </div>
